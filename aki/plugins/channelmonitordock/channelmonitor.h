...
class ChannelMonitor : public KTextBrowser
{
    Q_OBJECT
public:
    ChannelMonitor(QWidget *parent = 0);
    ~ChannelMonitor();
    void addMessage(const QString &channel, const QString &from, const QString &message);
    void addMessageHighlight(const QString &channel, const QString &from, const QString &message);
    void addCtcpAction(const QString &channel, const QString &from, const QString &message);
    void clear();
private:
    friend class ChannelMonitorPrivate;
    ChannelMonitorPrivate* const d;
}; // End of class ChannelMonitor.

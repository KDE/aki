    ...
    : KConfigDialog(parent, "settings", Aki::Settings::self())
{
    ...
    addPage(new ColorPage(this));
    addPage(new PluginPage(this));
    addPage(new ServerPage(this));
    addPage(new IrcPage(this));
...
SettingsDialog::addPage(Aki::SettingsPageInterface *page)
{
    ...
    m_pageList.insert(page, item);
}

void
SettingsDialog::removePage(Aki::SettingsPageInterface *page)
{
    QMutableMapIterator<Aki::SettingsPageInterface*, KPageWidgetItem*> pageIter(m_pageList);
...
{
    bool changed = false;
    foreach (Aki::SettingsPageInterface *page, m_pageList.keys()) {
        if (page->hasChanged()) {
            changed = true;
        }
    }

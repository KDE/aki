...
{
    QSqlDatabase db = QSqlDatabase::addDatabase(QLatin1String("QSQLITE3"));
    db.setDatabaseName(path);

    if (!db.open()) {
        qxtLog->error() << QString(QLatin1String("Unable to open database file.")).arg(db.lastError().text());
        return false;
    }

    QSqlQuery query(QLatin1String("PRAGMA foreign_keys=ON"));
    if (!query.exec()) {
        Aki::DatabasePrivate::checkError(query.lastError());
        qxtLog->error() << QLatin1String("Foreign keys not enabled");
    }
...
Database::createDefaultTables()
{
    ...
    QString tables = QLatin1String("CREATE TABLE IF NOT EXISTS Identity (\n"
    ...
    tables = QLatin1String("CREATE TABLE IF NOT EXISTS Server (\n"
                           "    id INTEGER PRIMARY KEY AUTOINCREMENT,\n"
    ...
    tables = QLatin1String("CREATE TABLE IF NOT EXISTS Address (\n"
                           "    id INTEGER PRIMARY KEY AUTOINCREMENT,\n"
    ...
    tables = QLatin1String("CREATE TABLE IF NOT EXISTS Channel (\n"
                           "    id INTEGER PRIMARY KEY AUTOINCREMENT,\n"
    ...
    tables = QLatin1String("CREATE TABLE IF NOT EXISTS Nickname (\n"
                           "    id INTEGER PRIMARY KEY AUTOINCREMENT,\n"
    ...
    QSharedPointer<Aki::SqlIdentity> defaultIdentity(Aki::SqlIdentity::newIdentity(QLatin1String("Default Identity")));
    if (defaultIdentity.isNull()) {
        return false;
    }

    QScopedPointer<Aki::SqlServer> defaultServer(Aki::SqlServer::newServer(QLatin1String("Freenode"), defaultIdentity.data()));
    if (defaultServer.isNull()) {
        return false;
    }

    QScopedPointer<Aki::SqlChannel> defaultChannel(Aki::SqlChannel::newChannel(QLatin1String("#aki"), defaultServer.data()));
    if (defaultChannel.isNull()) {
        return false;
    }

    QScopedPointer<Aki::SqlAddress> defaultAddress(Aki::SqlAddress::newAddress(QLatin1String("chat.freenode.net"),
                                                                               defaultServer.data()));
    ...
    delete Aki::SqlNickname::newNickname(user.loginName() + QLatin1Char('_'), defaultServer.data());
    delete Aki::SqlNickname::newNickname(user.loginName() + QLatin1String("__"), defaultServer.data());
